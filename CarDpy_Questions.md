...
- **Possible fix:** set `dif = 0` (matches the `static`/`moving` selection just above). Changes which image is registered.

Note AJH: made the changes  to both 
...
AJH: made them both match --> we should make note in the readme 
...
- **Possible fix:** `if dif in bval_low_indicies:`what should ADC filtering do with multiple b=0 volumes (skip all, or average before computing `S_0`)? Line 38 already hardcodes `bval_low_indicies[0]` as `S_0`, so intended behavior is unclear.

AJH: changed Diffusivity.py:31 to "if dif in bval_low_indicies:" rather than "if dif == bval_low_indicies:"
...
AJH: Dahnia added a check for the matrix size 